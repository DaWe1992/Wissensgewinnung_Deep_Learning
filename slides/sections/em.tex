...
\begin{frame}{Problemstellung (Forts.)}{}
	\begin{itemize}
		\item Mit $\{ \bX, \bZ \}$ bezeichnen wir den \keyword{vollständigen} Datensatz, $\bX$ alleine nennen wir
			hingegen \keyword{unvollständig}.
		\item Analog nennen wir $\ln p(\bX, \bZ \param \btheta)$ die \keyword{vollständige Log-Likelihood} und
			$\ln p(\bX \param \btheta)$ die \keyword{unvollständige Log-Likelihood}.
	\end{itemize}
\end{frame}
...
\begin{frame}{Expectation-Maximization (EM)}{}
	...
	\begin{boxGray}
		\keyword{E-Phase:} Bestimme den Erwartungswert von $\ln p(\bX, \bZ \param \btheta)$ unter der Posterior-Verteilung
		der latenten Variablen n$p(\bZ \mid \bX \param \bthetaOld)$. Die Erwartungswertfunktion bildet eine
		untere Schranke für $\ln p(\bX \param \btheta)$.
	\end{boxGray}
...
\begin{frame}{Aufgabe: EM für Probleme mit fehlenden Daten}{}
	\begin{mytask}[task:em-missing-data]
		...
		Eine Münze mit unbekanntem Parameter $\theta$ (Wahrscheinlichkeit für Kopf) wird $N = 3$ Mal geworfen.
		Leider wurden nur die ersten zwei Würfe beobachtet, der dritte Wurf ging verloren (latente Variable). Der erste Wurf sei $x_1 = 1$ (Kopf)
		und der zweite Wurf $x_2 = 0$ (Zahl).
		
		Führen Sie ausgehend vom Startwert $\thetaOld = 0.7$ eine vollständige Iteration des EM-Verfahrens durch.
		Wie lautet der neue Parameter $\thetaNew$?
	\end{mytask}
\end{frame}
...
\begin{frame}{Gaussian Mixture Models und EM}{}
	...
	\begin{itemize}
		\item Im \textit{E-Schritt} benötigen wir die Posterior-Verteilung $p(\bZ \mid \bX \param \btheta)$ der latenten Variablen, wobei
			$\btheta \coloneqq \bigl( \bpi, \bmu, \bSigma \bigr) \coloneqq
			\bigl( \pi_1, \ldots, \pi_K, \bmu_1, \ldots, \bmu_K, \bSigma_1, \ldots, \bSigma_K \bigr)$ ist.
		\item Diese können wir mit \eqref{eq:gmm-marginal-z}, \eqref{eq:gmm-x-given-z} und dem \textit{Satz von Bayes} bestimmen zu
		\begin{equation*}
			p(\bZ \mid \bX \param \btheta)
				\propto \prod_{n=1}^N \prod_{k=1}^K \bigl[ \pi_k \cdot \normdense(\bx_n \param \bmu_k, \bSigma_k) \bigr]^{z_{nk}}.
		\end{equation*}
		\item Als nächstes stellen wir die \textit{erwartete vollständige Log-Likelihood} auf, wobei der Erwartungswert bezüglich der
			Posterior-Verteilung $p(\bZ \mid \bX \param \btheta)$ zu berechnen ist.
		...
		\begin{equation} \label{eq:gmm-latent-var-expected-value}
			\begin{aligned}
				\E_{\bZ \sim p(\bZ\,\mid\,\bX \param \btheta)}[z_{nk}]
					&= \frac{
						\displaystyle\sum_{\bz_n} z_{nk} \prod_{k'=1}^K \bigl[ \pi_{k'}
							\cdot \normdense(\bx_n \param \bmu_{k'}, \bSigma_{k'}) \bigr]^{z_{nk'}}
					}{
						\displaystyle\sum_{\bz_n} \prod_{j=1}^K \bigl[ \pi_j
							\cdot \normdense(\bx_n \param \bmu_j, \bSigma_j) \bigr]^{z_{nj}}
					}
			\\[2mm]
					&= \frac{
						\pi_k \cdot \normdense(\bx_n \param \bmu_k, \bSigma_k)
					}{
						\displaystyle\sum_{j=1}^K \pi_j \cdot \normdense(\bx_n \param \bmu_j, \bSigma_j)
					}
						= \gamma(z_{nk}).
			\end{aligned}
		\end{equation}
		\item Der Erwartungswert der Indikator-Variablen $z_{nk}$ unter der Posterior-Verteilung von $\bZ$
			stellt sich also gerade als die jeweilige Responsibility $\gamma(z_{nk})$ heraus.
	\end{itemize}
\end{frame}
...
% Gaussian Mixture Models und EM (Forts.)
\begin{frame}{Gaussian Mixture Models und EM (Forts.)}{}
	Hiermit können wir nun die \textit{erwartete vollständige Log-Likelihood} (Funktion $Q$) aufstellen
...
\begin{frame}{Gaussian Mixture Models und EM (Forts.)}{}
	\begin{itemize}
		\item Die Funktion \eqref{eq:gmm-expected-complete-log-likelihood} müssen wir nun im \textit{M-Schritt} bezüglich der Parameter maximieren.
		\item \textbf{Beispielhaft:} Gradient bezüglich $\bmu_k$
		\begin{align*}
			\frac{\partial}{\partial \bmu_k} \sum_{n=1}^N \sum_{j=1}^K \gamma(z_{nj})
			\cdot \bigl\{ \ln \pi_j + \ln \normdense(\bx_n \param \bmu_j, \bSigma_j) \bigr\}
				&= \sum_{n=1}^N \gamma(z_{nk}) \cdot \frac{\partial}{\partial \bmu_k} \ln \normdense(\bx_n \param \bmu_k, \bSigma_k)
		\\[2mm]
				&= \sum_{n=1}^N \gamma(z_{nk}) \cdot \frac{
					\normdense(\bx_n \param \bmu_k, \bSigma_k)
				}{
					\normdense(\bx_n \param \bmu_k, \bSigma_k)} \cdot \bSigma_k^{-1} (\bx_n - \bmu_k)
		\\[2mm]
...
\begin{frame}{Gaussian Mixture Models und EM (Forts.)}{}
	\begin{itemize}
		\item Nullsetzen und Auflösen nach $\bmu_k$ ergibt
		\begin{equation*}
			\bmu_k = \frac{1}{N_k} \sum_{n=1}^N \gamma(z_{nk}) \cdot \bx_n.
		\end{equation*}
		\item Dieses Ergebnis hat dieselbe Form wie \eqref{eq:gmm-update-means}.
