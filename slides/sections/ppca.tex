...
\begin{frame}{Anwendung des EM-Verfahrens}{}
	\begin{itemize}
		...
		\item \textbf{Zur Erinnerung:} Im E-Schritt müssen wir
			$\E_{\bZ \sim p(\bZ\,\mid\,\bX)}\left[ \ln p(\bX, \bZ \param \btheta) \right]$ bestimmen.
		\item Mit dem \textit{Satz von Bayes} und der Linearität des Erwartungswerts können wir schreiben
		\begin{equation}
			\E_{\bZ \sim p(\bZ\,\mid\,\bX)}\left[ \ln p(\bX, \bZ \param \btheta) \right]
				\propto \E_{\bZ \sim p(\bZ\,\mid\,\bX)}\left[ \ln p(\bZ) \right]
					+ \E_{\bZ \sim p(\bZ\,\mid\,\bX)}\left[ \ln p(\bX \mid \bZ) \right]
		\end{equation}
