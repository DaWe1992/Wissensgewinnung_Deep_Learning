...
\begin{frame}{Wahrscheinlichkeitsräume}{}
	...
	\begin{itemize}
		...
		\item Mit $\sA$ bezeichnen wir die zugehörige \keyword{Sigma-Algebra}, eine Menge von Teilmengen von $\Omega$.
			...
			denen keine Wahrscheinlichkeit zugeordnet werden kann. Die Sigma-Algebra $\B$ auf $\R$ heißt
			\keyword{Borel-Sigma-Algebra}.
...
\begin{frame}{Zufallsvariablen (Forts.)}{}
	...
	\begin{boxGray}
		\textbf{Beispiel:}
		$X$ ist $\norm(\mu, \sigma^2)$-verteilt genau dann,
		wenn es einen Wahrscheinlichkeitsraum $(\Omega, \sA, \Prob)$ gibt,
...
\begin{frame}{Wahrscheinlichkeitsdichten}{}
	\begin{mydef}[def:density]
		...
		Wir nennen eine nichtnegative Funktion $$p_X : \R \rightarrow [0, \infty)$$ eine \keyword{(Wahrscheinlichkeits-)Dichte}
		von $\Prob^X$, falls
		\begin{equation*}
...
\begin{frame}{Erwartungswert von Zufallsvariablen}{}
	...
	Aus der Linearität des Integrals folgt die des Erwartungswerts, d.\,h. für zwei Zufallsvariablen $X$ und $Y$
	und eine beliebige aber feste Zahl $\alpha \in \R$ (keine Zufallsvariable!) gelten
	\begin{equation*}
...
\begin{frame}{Varianz von Zufallsvariablen}{}
	\begin{mydef}[def:variance]
		Die Varianz einer Zufallsvariablen $X \in \R$ ist definiert als die \textit{erwartete quadratische Abweichung
		vom Erwartungswert} (zweites zentriertes Moment), also
		\begin{equation*}
...
\begin{frame}{Varianz von Zufallsvariablen (Forts.)}{}
	...
	\begin{boxGray}
		Eine Verschiebung verändert die Varianz nicht, also $\V[X + \alpha] = \V[X]$ ($\alpha$ ist hierbei keine Zufallsvariable,
		sondern eine konstante reelle Zahl).
	\end{boxGray}
\end{frame}
...
\begin{frame}{Kovarianz}{}
	...
	\begin{mytask}[task:stat-cov-cor]
...
\begin{frame}{Streuungsmatrix}{}
	...
	\begin{mytask}[task:stat-scatter-matrix-psd]
...
\begin{frame}{Stochastische Unabhängigkeit}{}
	...
	\begin{mydef}[def:independence]
		Es seien $X \in \R$ und $Y \in \R$ reelle Zufallsvariablen mit den Verteilungen $\Prob^X, \Prob^Y$
		und den Dichten $p_X$ bzw. $p_Y$.
	...
	\begin{boxGray}
		\textbf{Bemerkung:} Stochastisch unabhängige Zufallsvariablen sind auch unkorreliert.
		\textbf{Die Umkehrung gilt im Allgemeinen nicht!}
...
\begin{frame}{Bedingte Wahrscheinlichkeiten}{}
	...
	\begin{mydef}[def:cond-prop]
		...
		Dann ist die \keyword{Wahrscheinlichkeitsdichte von $X$ gegeben $Y$}, in Notation $p_{X\,\mid\,Y}(x \mid y)$,
		definiert als
		\begin{equation*}
...
\begin{frame}{Satz von Bayes}{}
	...
	\begin{mytheo}[th:bayes]
		...
		\begin{equation*}
			p_{X\,\mid\,Y}(x \mid y) = \frac{p_{Y\,\mid\,X}(y \mid x) \cdot p_X(x)}{p_Y(y)}.
		\end{equation*}
	\end{mytheo}

	\begin{myproof}
		Mit Definition \ref{def:cond-prop} gilt $p_{X\,\mid\,Y}(x \mid y) = \frac{p_{X, Y}(x, y)}{p_Y(y)}$.
		Analog gilt $p_{Y\,\mid\,X}(y \mid x) = \frac{p_{Y, X}(y, x)}{p_X(x)}$.
		Da $p_{X, Y}(x, y) = p_{Y, X}(y, x)$, können wir beide Gleichungen nach der Verbunddichte $p_{X, Y}(x, y)$ auflösen.
		Wir erhalten also
		$p_{X, Y}(x, y) = p_{X\,\mid\,Y}(x \mid y) \cdot p_Y(y)$ und $p_{X, Y}(x, y) = p_{Y\,\mid\,X}(y \mid x) \cdot p_X(x)$.
		Gleichsetzen liefert schließlich
		\begin{equation*}
			p_{X\,\mid\,Y}(x \mid y) \cdot p_Y(y) = p_{Y\,\mid\,X}(y \mid x) \cdot p_X(x)
...
\begin{frame}{Transformationssatz für Lebesgue-Dichten}{}
	\begin{mytheo}[th:stat-transformation]
...
\begin{frame}{Beispiel II}{}
	...
	\begin{boxGray}
		...
		Die Anwendung der Transformationsformel \ref{th:stat-transformation} ergibt daher für die Dichte $p_{\by}$ von $\by$
		den Ausdruck
		\begin{equation*}
...
\begin{frame}{Beispiel III}{}
	...
	\begin{mytask}[task:stat-transformation]
...
\begin{frame}{Multivariate Normalverteilung}{}
	...
	\begin{mytheo}[th:std-norm-mv]
		Es sei $\bx \coloneqq (x_1, x_2, \ldots, x_N)^\top \in \R^N$ ein Zufallsvektor, dessen Komponenten
		$x_i \in \R$ unabhängig und identisch standardnormalverteilt sind,
		d.\,h. $x_i \sim \norm(0, 1)\,\forall i \in \{ 1, \ldots, N \}$.
		
		Die Dichte der \keyword{multivariaten Standardnormalverteilung} ist dann gegeben durch
		\begin{equation*}
			\normdense_{\bx}(\bx) \coloneqq \frac{1}{(2 \pi)^{\frac{N}{2}}} \cdot \exp\left( -\frac{\Vert \bx \Vert^2}{2} \right).
		\end{equation*}
	\end{mytheo}
...
\begin{frame}{Multivariate Normalverteilung (Forts.)}{}
	...
	Wir verallgemeinern dies auf beliebige Erwartungswertvektoren $\bmu \in \R^N$ und Streuungsmatrizen
	...
	\begin{enumerate}
		\item Aufgrund der positiven Definitheit von $\bSigma$ gibt es eine untere Dreiecksmatrix $\bA$ mit
			$\bSigma = \bA \bA^\top$ (Stichwort: \keyword{Cholesky-Zerlegung}).
		...
		\end{equation*}
		\item Nun wenden wir die Transformationsformel \ref{th:stat-transformation} an, um die Dichte von $\by$ zu bestimmen.
	\end{enumerate}
\end{frame}
...
\begin{frame}{Multivariate Normalverteilung (Forts.)}{}
	...
	\begin{equation*}
		p_{\by}(\by) = \frac{1}{(\det \bSigma)^\frac{1}{2}} \cdot \normdense_{\bx}\bigl( \bA^{-1} (\by - \bmu) \bigr)
	\end{equation*}
	...
	$\Vert \bx \Vert^2 = (\by - \bmu)^\top \bigl(\bA^{-1})^\top \bA^{-1} (\by - \bmu) = (\by - \bmu)^\top \bSigma^{-1} (\by - \bmu)$
	gilt. 
\end{frame}
...
\begin{frame}{Multivariate Normalverteilung (Forts.)}{}
	...
	\begin{mydef}[def:norm-mv]
		...
		\begin{equation*}
			\normdense_{\bx}(\bx) \coloneqq \frac{1}{(2 \pi)^{\frac{N}{2}} \cdot (\det \bSigma)^{\frac{1}{2}}} \cdot \exp\left(
	...
	\begin{boxGray}
		...
		(siehe hierzu Satz \ref{th:reg-col-rank-inv}).
	\end{boxGray}
\end{frame}
...
\begin{frame}{Visualisierung: Multivariate Normalverteilung}{}
	\begin{figure}[H]
		\divideThree{0.32}{
			\centering
			\includegraphics[scale=1.0]{./img/statistics/gaussian_general.pdf}			
		}{0.32}{
			\centering
			\includegraphics[scale=1.0]{./img/statistics/gaussian_diag.pdf}
		}{0.32}{
			\centering
			\includegraphics[scale=1.0]{./img/statistics/gaussian_prop_identity.pdf}
		}
		\caption{
			Multivariate (genauer zweidimensionale) Normalverteilung mit (Links): Allgemeiner Kovarianzmatrix.
			(Mitte): Diagonaler Kovarianzmatrix, sodass die Konturen achsenparallel verlaufen.
			(Rechts): Kovarianz proportional zur Einheitsmatrix, also $a \bI$ mit $a > 0$,
			sodass die Konturen perfekte Kreise bilden.
			Siehe \cite{Bishop2024}, Seite 76.}
	\end{figure}
\end{frame}
...
\begin{frame}{Partitionierte Normalverteilung}{}
	...
	\begin{boxGray}
		...
		Oft ist es einfacher mit der Inversen der Kovarianzmatrix zu arbeiten. Wir definieren daher
		$\bLambda \coloneqq \bSigma^{-1}$. Man nennt diese Matrix \keyword{Präzisionsmatrix}.
	\end{boxGray}
\end{frame}
...
\begin{frame}{Bedingte Normalverteilung}{}
	\begin{mytheo}[th:stat-cond-norm]
...
\begin{frame}{Visualisierung: Bedingte Normalverteilung}{}
	\begin{figure}[H]
		...
		\caption{Links: Zweidimensionale Normalverteilung. Rechts: Bedingte Verteilung in rot.
		Siehe \cite{Bishop2024}, Seite 82.}
	\end{figure}
\end{frame}
...
\begin{frame}{Aufgabe: Bedingte Normalverteilung}{}
	\begin{mytask}[task:stat-cond-norm]
		Es seien $X_1$ und $X_2$ zwei Zufallsvariablen mit $\bigl( X_1, X_2 \bigr)^\top \sim \norm(\bmu, \bSigma)$.
		Hierbei sind die Parameter gegen durch $\bmu \coloneqq \bigl( 0, 2 \bigr)^\top \in \R^2$ und
		\begin{equation*}
			\bSigma \coloneqq \begin{pmatrix} 0.3 & -1 \\ -1 & 5 \end{pmatrix} \in \R^{2 \times 2}.
		\end{equation*}
		Bestimmen Sie die bedingte Dichte $p_{X_1\,\mid\,X_2}(x_1 \mid x_2 = -1)$.
		Benutzen Sie hierzu die Formeln aus Satz \ref{th:stat-cond-norm}.
	\end{mytask}
\end{frame}
...
\begin{frame}{Normale Randverteilung}
	\begin{mytheo}[th:stat-norm-marginal]
		...
		\end{align*}
		Die Dichten der Randverteilung bezeichnen wir mit $n_{\bx}$ und $n_{\by}$.
	\end{mytheo}

	\begin{myproof}
		Man erhält dieses Ergebnis durch \keyword{Marginalisierung} der Verbunddichte,
		d.\,h. wir integrieren über alle Variablen, die uns nicht interessieren. Für die Randverteilung von $\bx$ gilt zum Beispiel
		\begin{equation*}
			\normdense_{\bx}(\bx) = \int \normdense_{\bx, \by}(\bx, \by)\,\diff \by.
		\end{equation*}
...
\begin{frame}{Visualisierung: Normale Randverteilung}{}
	\begin{figure}[H]
		...
		\caption{Links: Zweidimensionale Normalverteilung. Rechts: Randverteilung für $x_a$ in blau.
		Siehe \cite{Bishop2024}, Seite 82.}
	\end{figure}
\end{frame}
...
\begin{frame}{Aufgabe: Normale Randverteilung}{}
	\begin{mytask}[task:stat-norm-marginal]
		Betrachten Sie erneut die Situation aus Aufgabe \ref{task:stat-cond-norm}.
		
		Wie lautet die Dichte der Randverteilung $p_{X_1}(x_1)$?
	\end{mytask}
\end{frame}
...
\begin{frame}{Satz von Bayes für Normalverteilungen}{}
	\begin{mytheo}[th:stat-bayes-norm]
		Es seien $\bx$ und $\by$ Zufallsvektoren, sodass gilt
		\begin{equation*}
			\bx \sim \norm(\bmu, \bSigma)
		\quad\text{und}\quad
			\by \mid \bx \sim \norm(\bA \bx + \bb, \bLambda).
		\end{equation*}
		Dann gilt für die Dichten der Verteilungen von $\by$ und $\bx \mid \by$
		\begin{align*}
			\by
		...
		\\[2mm]
			\bx \mid \by
				&\sim \norm\Bigl(
					\bGamma \bigl[ \bSigma^{-1} \bmu + \bA^\top \bLambda^{-1} (\by - \bb) \bigr],
					\bGamma
				\Bigr).
		\end{align*}
	...
	\begin{myproof}
		Einen Beweis dieser Aussage finden Sie in \cite{Bishop2006} auf den Seiten 90--93.
	\end{myproof}
\end{frame}
...
\begin{frame}{Maximum-Likelihood Methode}{}
	\begin{itemize}
		\item Im Folgenden sei ein Beobachtungsvektor $\bx \coloneqq \bigl( x_1, x_2, \ldots, x_N \bigr)^\top$
			bestehend aus $N$ \textit{unabhängig und identisch verteilten}
			(Englisch: \textit{independent and identically distributed} - i.\,i.\,d.) Datenpunkten gegeben.
		...
		\item Dies erledigen wir durch eine Punktschätzung des Parametervektors $\btheta$.
			Den Schätzer für $\btheta$ bezeichnen wir mit $\bthetaest$.
...
\begin{frame}{(Log-)Likelihood Funktion}{}
	...
	\begin{mydef}[def:likelihood-function]
		Es sei $p(\bx; \btheta)$ die Dichte eines Zufallsvektors $\bx \in \R^N$, welche durch den
		Parametervektor $\btheta \in \Theta$ parametrisiert wird.
...
\begin{frame}{Beispiel II: Normalverteilung}{}
	\begin{mytheo}[th:mle-norm]
		...
		Der Parametervektor $\btheta$ ist in diesem Fall zweielementig und gegeben durch
		$\btheta \coloneqq \bigl( \mu, \sigma^2 \bigr)^\top$.
		
		Die Maximum-Likelihood Schätzer $\muML$ und $\sigmasquML$ für die unbekannten Parameter
		$\mu$ und $\sigma^2$ werden in diesem Fall bestimmt mittels
		\begin{equation*}
			\muML
	...
	\begin{myproof}
		...
		\begin{equation*}
			p\bigl( \bx; \mu, \sigma^2 \bigr)
				...
				= \frac{1}{(2 \pi)^{\frac{N}{2}}} \cdot \frac{1}{\sigma^N}
					\cdot \exp\left( -\frac{\sum_{n=1}^N (x_i - \mu)^2}{2 \sigma^2} \right).
		\end{equation*}
		Aufgrund der auftretenden Exponentialverteilung bietet sich die Betrachtung der Log-Likelihood Funktion an.
		Wir erhalten
		\begin{equation*}
		...
		\end{equation*}
		Durch Nullsetzen und auflösen erhalten wir die im Satz angegebenen Maximum-Likelihood Schätzer
		$\muML$ und $\sigmasquML$.
		Es bleibt noch zu zeigen, dass es sich tatsächlich um ein Maximum handelt. Man rechnet nach,
		dass die Hesse-Matrix von $l\bigl( \mu, \sigma^2 \bigr)$ and der Stelle $\bigl( \muML, \sigmasquML \bigr)$
		gegeben ist durch
		\begin{equation*}
...
\begin{frame}{Aufgabe zur Maximum-Likelihood Schätzung}{}
	\begin{mytask}[task:stat-ml-poisson]
...
\begin{frame}{Binäre Variablen}{}
	...
	\begin{mytask}[task:stat-expect-var-bernoulli]
...
\begin{frame}{Aufgabe: Konjugierte Verteilungen}{}
	\begin{mytask}[task:stat-conjug]
		Weisen Sie nach, dass die Gamma-Verteilung der konjugierte Prior zur Likelihood der Poisson-Verteilung für den Parameter $\lambda$ ist.
		
		\textbf{Hinweis:} Schreiben Sie hierzu die Likelihood-Funktion und den Prior für $\lambda$ auf. Ignorieren Sie alle Terme, die nicht von
		$\lambda$ abhängen. Wenden Sie anschließend den Satz von Bayes an, um den Posterior zu erhalten. Können Sie diesen in dieselbe
		Form wie den Prior bringen? 
	\end{mytask}
\end{frame}
