...
\begin{frame}{Geometrisches Skalarprodukt}{}
	...
	\begin{myproof}
		...
		\begin{equation} \label{eq:pca-geom-scalar-product-1}
			...
			\end{aligned}
		\end{equation}
		und zum Anderen aufgrund des \keyword{Kosinussatzes} (Verallgemeinerung des Satzes des Pythagoras) aus der Geometrie
...
\begin{frame}{Orthogonalität von Vektoren}{}
	...
	\begin{myproof}
		...
		da die Kosinus-Funktion bei $\pi/2$ (entspricht 90°) eine Nullstelle hat. Mit Satz \ref{th:pca-geom-scalar-product} folgt
		\begin{equation*}
			\ba^\top \bb
...
\begin{frame}{Orthogonale Projektion von Vektoren (Forts.)}{}
	\begin{figure}
		...
		\caption{%
			Projektion des Vektors $\bb \in \R^2$ auf den Vektor $\ba \in \R^2$.
	...
	\begin{itemize}
		...
		\begin{equation}
			\Vert \proj_{\ba} \bb \Vert
				= \frac{\vert \ba^\top \bb \vert}{\Vert \ba \Vert^2} \Vert \ba \Vert
				= \frac{\vert \ba^\top \bb \vert}{\Vert \ba \Vert}.
		\end{equation}
		...
		\begin{equation}
			\Vert \proj_{\ba} \bb \Vert = \vert \ba^\top \bb \vert.
		\end{equation}
...
\begin{frame}{Projektion von Mittelwert und Varianz}{}
	\begin{itemize}
		\item Der \keyword{Mittelwert} der projizierten Daten ist analog gegeben durch
...
\begin{frame}{Maximierung der projizierten Varianz}{}
	\begin{itemize}
		...
		\item \textbf{Problem:} Dieses Maximierungsproblem ist \textit{unbeschränkt}.
...
\begin{frame}{Maximierung der projizierten Varianz (Forts.)}{}
	...
	\begin{mytask}
		...
		und finden Sie deren stationäre Punkte.
	\end{mytask}
\end{frame}
...
\begin{frame}{Bestimmung weiterer Principal Components}{}
	\begin{itemize}
		...
		\item Wir fassen die Vektoren $\bu_1, \bu_2, \ldots, \bu_D$ dann spaltenweise in einer Matrix $\bV \in \R^{M \times D}$ zusammen.
...
\begin{frame}{Orthogonalität der Principal Components}{}
	...
	\begin{myproof}
		Dies folgt direkt aus der Orthogonalität der Eigenvektoren symmetrischer Matrizen zu verschiedenen Eigenwerten.
		Seien also $\lambda_1$ und $\lambda_2$ mit $\lambda_1 \ne \lambda_2$ zwei voneinander verschiedene Eigenwerte
		von $\bS$. Dann erfüllen diese die Gleichungen $\bS \bu_1 = \lambda_1 \bu_1$ und $\bS \bu_2 = \lambda_2 \bu_2$. Damit gilt
...
\begin{frame}{Minimum-Error Formulierung}{}
	\begin{itemize}
		\item Wir betrachten im Folgenden eine weitere Formulierung der PCA, welche auf der
			\textit{Minimierung des Projektionsfehlers} basiert, die \keyword{Minimum-Error} Formulierung.
		\item Dazu seien $\{ \bu_i \}$ mit $i = 1, 2, \ldots, M$ \textit{orthonormale Basisvektoren}, d.\,h.
		\begin{equation} \label{eq:pca-orthonormality}
			\bu_i^\top \bu_j = \delta_{ij} \coloneqq \begin{cases}
				1 & \text{if}\quad i= j 		\\
				0 & \text{if}\quad i \ne j
			\end{cases} \qquad \forall\,i, j.
		\end{equation}
		\item Die Basis $\{ \bu_i \}$ ist \textit{vollständig}, weshalb sich jedes $\bx_n$ \textit{eindeutig} als Linearkombination
			der $\bu_i$ darstellen lässt
...
\begin{frame}{Projektionsfehler}{}
	\begin{itemize}
		...
		\item Diesen definieren wir als die \textit{mittlere quadratische Abweichung} der $\{ \bx_n \}$ und den $\{ \bxtilde_n \}$.
	\end{itemize}
...
\begin{frame}{Minimierung des Projektionsfehlers (Forts.)}{}
	\begin{itemize}
		...
		\end{equation}
		\item Dies substituieren wir schließlich in die Fehlerfunktion \eqref{eq:pca-projection-error}.
		\item Es gilt
...
\begin{frame}{Minimierung des Projektionsfehlers (Forts.)}{}
	\begin{itemize}
		...
		\item Die Nebenbedingungen ergeben sich aus der vorausgesetzten \textit{Orthonormalität}
			\eqref{eq:pca-orthonormality} der $\{ \bu_i \}$.
...
\begin{frame}{Minimierung des Projektionsfehlers (Forts.)}{}
	\begin{itemize}
		\item Der Fehler wird in diesem Fall also minimal,
			wenn man für die Projektion einen Eigenvektor zum größten Eigenwert benutzt.
		\item Für allgemeines $M$ und $D$ erhält man den minimalen Fehler $J$, indem man für die Projektion normierte
		 	Eigenvektoren von $\bS$ zu den $D$ größten Eigenwerten wählt.
		\item Für alle $i = 1, 2, \ldots, M$ müssen die $\{ \bu_i \}$ also erfüllen
		\begin{equation}
			\bS \bu_i = \lambda_i \bu_i.
		\end{equation}
		\item Der Fehler $J$ ergibt sich dann als Summe der Eigenwerte jener Eigenvektoren,
			die für die Projektion nicht berücksichtigt wurden.
		\begin{equation}
