...
\begin{frame}{Gaussian Mixture Models}{}
	\begin{itemize}
		...
		\begin{equation} \label{eq:gmm-density}
			p(\bx; \btheta) \coloneqq \sum_{k=1}^K \pi_k \cdot p(\bx; \bmu_k, \bSigma_k).
		\end{equation}
...
\begin{frame}{Gaussian Mixture Models als Latent Variable Models}{}
	\begin{itemize}
		...
		\end{equation*}
		\item Für die Definition der Dichte $p(\bz)$ benutzen wir die Mischkoeffizienten: $$p(z_k = 1; \pi_k) \coloneqq \pi_k.$$
...
\begin{frame}{Gaussian Mixture Models als Latent Variable Models (Forts.)}{}
	\begin{itemize}
		...
		\begin{equation*}
			p(\bx \mid z_k = 1; \bmu_k, \bSigma_k) \coloneqq p(\bx; \bmu_k, \bSigma_k).
		\end{equation*}
		\item Dies können wir ebenfalls vektorisiert ausdrücken als
		\begin{equation} \label{eq:gmm-x-given-z}
			p(\bx \mid \bz; \bmu, \bSigma) = \prod_{k=1}^K p(\bx; \bmu_k, \bSigma_k)^{z_k}.
		\end{equation}
		\item Die Randdichte $p(\bx; \btheta)$ erhalten wir schließlich durch Marginalisierung in $\bz$
		\begin{equation}
			p(\bx; \btheta)
				= \sum_{\bz} p(\bz; \bpi) \cdot p(\bx \mid \bz; \bmu, \bSigma)
				= \sum_{k=1}^K \pi_k \cdot p(\bx; \bmu_k, \bSigma_k).
		\end{equation}
...
\begin{frame}{Latente Variablen}{}
	\begin{itemize}
		...
		\item Allerdings können wir nun mit der gemeinsamen Dichte $p(\bx, \bz; \btheta)$ anstelle von $p(\bx; \btheta)$ arbeiten,
...
\begin{frame}{Responsibilities}{}
	\begin{itemize}
		\item Nachfolgend interessieren wir uns besonders für die Posterior-Dichte $p(\bz \mid \bx; \btheta)$.
...
\begin{frame}{EM Algorithmus für GMMs}{}
	\begin{myalg}
		\begin{enumerate}
			\item Initialisiere die Parameter $\{ \pi_k \}$, $\{ \bmu_k \}$ und $\{ \bSigma_k \}$ (entweder zufällig oder mit Vorwissen). 
