...
\begin{frame}{Kullback-Leibler Divergenz}{}
	\begin{itemize}
		\item Aus \eqref{eq:em-kl-div} ist ersichtlich, dass $\KL(q \parallel p)$ die Kullback-Leibler Divergenz zwischen
			$q(\bZ)$ und der Aposteriori-Verteilung $p(\bZ \mid \bX; \btheta)$ ist.
		\item \textbf{Eigenschaften der Kullback-Leibler Divergenz:} Für alle $q$ und $p$ gelten
		\begin{equation}
		\label{eq:kl-nicht-neg}
		...
		\end{equation}
		\item Aus \eqref{eq:zerlegung-unvollst-logl} folgt mit der Nichtnegativität \eqref{eq:kl-nicht-neg} der Kullback-Leibler Divergenz
...
\begin{frame}{Eine andere Sicht auf den E-Schritt}{}
	\begin{itemize}
		\item Wir nutzen die Zerlegung \eqref{eq:zerlegung-unvollst-logl}, um eine alternative Herleitung des EM-Algorithmus zu erhalten.
		\item Hierzu nehmen wir an, dass die aktuellen Parameter durch $\bthetaOld$ gegeben sind.
...
\begin{frame}{Eine andere Sicht auf den M-Schritt}{}
	\begin{itemize}
		...
		\item Setzen wir $p(\bZ) = p\bigl( \bZ \mid \bX; \bthetaOld \bigr)$ in die Definition der unteren Schranke \eqref{eq:elbo} ein, so erhalten wir
		\begin{align*}
			\sL(q, \btheta)
...
\begin{frame}{Eine andere Sicht auf den M-Schritt (Forts.)}{}
	\begin{itemize}
		\item Um die gegebene untere Schranke zu maximieren, müssen wir also die Funktion $Q\bigl( \btheta, \bthetaOld \bigr)$ maximieren,
			wobei $Q$ die erwartete vollständige Log-Likelihood ist.
		\item Damit nimmt der M-Schritt dieselbe Form wie in \eqref{eq:m-schritt} an
		\begin{equation*}
			\bthetaNew \coloneqq \argmax_{\btheta} Q\bigl( \btheta, \bthetaOld \bigr).
		\end{equation*}
		\item Weil die Verteilung $q$ im vorhergehenden E-Schritt mit den alten Parametern $\bthetaOld$ bestimmt wurde und diese
			für den gesamten M-Schritt fixiert wird, gilt $$q(\bZ) \ne p\bigl( \bZ \mid \bX; \bthetaNew \bigr),$$ sodass die Kullback-Leibler Divergenz
			zwischen diesen beiden Verteilung größer als 0 ist.
...
\begin{frame}{Gaussian Mixture Models und EM}{}
	\begin{itemize}
		\item Im E-Schritt benötigen wir die Aposteriori-Verteilung $p(\bZ \mid \bX; \bpi, \bmu, \bSigma)$ der latenten Variablen.
		\item Diese können wir mit \eqref{eq:gmm-marginal-z}, \eqref{eq:gmm-x-given-z} und dem Satz von Bayes bestimmen
		\begin{equation*}
			p(\bZ \mid \bX; \bpi, \bmu, \bSigma)
				\propto \prod_{n=1}^N \prod_{k=1}^K \bigl[ \pi_k \cdot \norm(\bx_n; \bmu_k, \bSigma_k) \bigr]^{z_{nk}}.
		\end{equation*}
		\item Diese Verteilung faktorisiert über $n$, sodass die latenten Variablen $\bz_n$ unabhängig sind.
		\item Als nächstes stellen wir die erwartete vollständige Log-Likelihood auf, wobei der Erwartungswert bezüglich der
			Aposteriori-Verteilung $p(\bZ \mid \bX; \bpi, \bmu, \bSigma)$ zu berechnen ist.
...
\begin{frame}{Gaussian Mixture Models und EM (Forts.)}{}
	\begin{itemize}
		...
		\begin{align*}
			\E_{\bZ \sim p(\bZ \mid \bX; \bpi, \bmu, \bSigma)}[z_{nk}]
				&= \frac{
		...
		\\[2mm]
				&= \frac{\pi_k \cdot \norm(\bx_n; \bmu_k, \bSigma_k)}{\displaystyle\sum_{j=1}^K \pi_j \cdot \norm(\bx_n; \bmu_j, \bSigma_j)}
					= \gamma(z_{nk}).
		\end{align*}
		\item Der Erwartungswert der Indikator-Variablen $z_{nk}$ unter der Aposteriori-Verteilung für $\bZ$
...
% Gaussian Mixture Models und EM (Forts.)
\begin{frame}{Gaussian Mixture Models und EM (Forts.)}{}
	Hiermit können wir nun die erwartete vollständige Log-Likelihood aufstellen
	\begin{align*}
		\E_{\bZ \sim p(\bZ \mid \bX; \bpi, \bmu, \bSigma)}\bigl[ \ln p(\bX, \bZ; \bpi, \bmu, \bSigma) \bigr]
			&= \E\left[
...
\begin{frame}{Gaussian Mixture Models und EM (Forts.)}{}
	\begin{itemize}
		\item Diese Funktion müssen wir nun im M-Schritt bezüglich der Parameter maximieren.
		\item \textbf{Beispielhaft:} Ableitung nach $\bmu_k$:
		\begin{align*}
		...
		\end{align*}
		\item Nullsetzen und Auflösen nach $\bmu_k$ ergibt $\displaystyle\bmu_k = \frac{1}{N_k} \sum_{n=1}^N \gamma(z_{nk}) \cdot \bx_n$.
	\end{itemize}
\end{frame}
