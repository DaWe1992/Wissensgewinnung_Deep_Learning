...
\begin{frame}{Gaussian Mixture Models}{}
	\begin{itemize}
		...
		\end{equation}
		\item Die \keyword{Mischkoeffizienten} $\pi_k$ stellen sicher, dass die Mixture-Dichte normiert ist, sodass also $\int_{-\infty}^{+\infty} p(\bx) = 1$ gilt.
...
\begin{frame}{Gaussian Mixture Models als Latent Variable Models (Forts.)}{}
	\begin{itemize}
		...
		\begin{equation}
		\label{eq:gmm-conditional-x-given-z}
		...
		\end{equation}
		\item Im letzten Gleichheitszeichen haben wir \eqref{eq:gmm-marginal-z} und \eqref{eq:gmm-conditional-x-given-z}
...
\begin{frame}{Latente Variablen}{}
	\begin{itemize}
		...
		\item Allerdings können wir nun mit der Verteilung $p(\bx, \bz)$ anstelle von $p(\bx)$ arbeiten, was zu wesentlichen Vereinfachungen führen wird.
...
\begin{frame}{Visualisierung: Responsibilities}{}
	\begin{figure}[H]
		...
		(c) Plot der Responsibilities. Dabei wurden die Farben im Verhältnis der Wahrscheinlichkeiten $\gamma(z_nk)$ gewählt. Siehe \cite{Bishop2024}, Seite 469.}
	\end{figure}
\end{frame}