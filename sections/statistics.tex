...
\begin{frame}{Maximum Likelihood für die Bernoulli-Verteilung}{}
	\begin{itemize}
		\item Wir gehen nun davon aus, dass $N$ unabhängige Bernoulli-verteilte Zufallsvariablen $X_1, X_2, \ldots, X_N$
			vorliegen.
		\item Dann ist die Likelihood Funktion für $\mu$ gegeben durch
		\begin{equation*}
		...
		\end{equation*}
		\item Wir sehen, dass die Log-Likelihood Funktion nur über die Statistik $Y \coloneqq \sum_{n=1}^N X_n$
			von den Beobachtungen abhängt. Man nennt $Y$ daher eine \keyword{suffiziente Statistik}.
	\end{itemize}
\end{frame}
...
\begin{frame}{Maximum Likelihood für die Bernoulli-Verteilung}{}
	\begin{itemize}
		...
		\item Die Zufallsvariable $Y \coloneqq X_1 + X_2 + \ldots + X_N$ ist dann aufgrund der Unabhängigkeit der $X_n$
			\keyword{binomial-verteilt}, sodass gilt
		\begin{equation*}
...
\begin{frame}{Bestimmung des Posteriors}{}
	\begin{itemize}
		\item Um den Posterior zu erhalten, multiplizieren wir nun den Beta-verteilten Prior mit der binomial-verteilten Likelihood.
		\item Ignorieren wir alle Terme, die nicht von $\mu$ abhängen, so erhalten wir für den Posterior den Ausdruck
		\begin{equation}
			p(\mu \mid m, l; a, b) \propto \mu^{m + a - 1} \cdot (1 - \mu)^{l + b - 1},
		\end{equation}
		wobei $l \coloneqq N - m$ die Anzahl der Misserfolge (Zahl) ist.
		\item Da Prior und Likelihood dieselbe funktionale Form haben, ist \textit{auch der Posterior wieder Beta-verteilt},
			wobei sich der Normierungsfaktor durch Vergleich mit Definition \ref{def:beta-dist} ergibt
		\begin{equation*}
			p(\mu \mid m, l; a, b)
				= \frac{\Gamma(m + a + l + b)}{\Gamma(m + a) \cdot \Gamma(l + b)}
					\cdot \mu^{m + a - 1} \cdot (1 - \mu)^{l + b - 1}
		\end{equation*}
		\item Diese Eigenschaft nennt man \keyword{Konjugiertheit}.
	\end{itemize}
\end{frame}
