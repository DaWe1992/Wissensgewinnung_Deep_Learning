...
\begin{frame}{Maximum-Likelihood Regression (Forts.)}{}
	\begin{itemize}
		\item Durch Logarithmieren von \eqref{eq:likelihood-reg} erhalten wir die Log-Likelihood Funktion
		\begin{equation}
			\label{eq:log-likelihood-reg}
			\begin{aligned}
			...
			\end{aligned}
		\end{equation}
		\item Aus \eqref{eq:log-likelihood-reg} wird erstichtlich, dass der Least-Squares-Fehler minimiert werden muss, um die
			Likelihood in $\btheta$ zu maximieren.
		\item Die Lösung für $\btheta$ ist dieselbe wie in \eqref{th:normal-equations}, zusätzlich kann die globale
...
\begin{frame}{Berücksichtigung einer Apriori-Verteilung}{}
	\begin{itemize}
		...
		\item Eine Möglichkeit der Regularisierung ist die Verwendung einer \keyword{Apriori-Verteilung} (Prior).
		\item Für diese Verteilung benutzen wir eine Normalverteilung
		\begin{equation}
